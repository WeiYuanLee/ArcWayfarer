...
export function UpdateModal({ isOpen, onClose, checkResult, loading, onRecheck }: Props) {
  ...
  return (
    <Modal opened={isOpen} onClose={onClose} title={t('version.title')} centered size="lg">
      <Stack gap="md">
        <Group justify="space-between"><Text size="sm" c="dimmed">{t('version.current')}: <Text span fw={600}>v{currentVersion}</Text></Text><Badge variant="light">{envLabels[currentEnv]}</Badge></Group>
        ...
        {!loading && !checkResult?.error && latestRelease && checkResult.hasUpdate && (
          <>
            <Alert color="blue" icon={<IconDownload size={16} />} title={`${t('version.new_available')} v${latestRelease.version}`}>
