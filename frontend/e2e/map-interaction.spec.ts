...
test.describe('Map Interaction', () => {
  ...
  test('地圖應包含縮放控制按鈕', async ({ page }) => {
    ...
    await expect(zoomIn).toHaveCSS('width', '46px')
    await expect(zoomIn).toHaveCSS('height', '46px')
  })

  test('兩種地圖模式的縮放控制尺寸應一致', async ({ page }) => {
    ...
    const leafletZoom = page.locator('.leaflet-control-zoom-in')
    await expect(leafletZoom).toHaveClass(/map-control-button/)
    await expect(leafletZoom).toHaveCSS('width', '46px')
    await expect(leafletZoom).toHaveCSS('height', '46px')
    await expect(page.locator('.leaflet-control-zoom')).toHaveCSS('margin', '0px')
    await leafletZoom.hover()
    await expect(leafletZoom).toHaveCSS('background-color', railHoverStyle.background)
    await expect(leafletZoom).toHaveCSS('border-color', railHoverStyle.border)
    await expect(leafletZoom).toHaveCSS('box-shadow', railHoverStyle.shadow)
    const leafletBox = await leafletZoom.boundingBox()

    await modeButton.click()
    await page.getByRole('menuitemradio', { name: /高效能模式/ }).click()
    const mapLibreZoom = page.locator('.maplibregl-ctrl-zoom-in')
    await expect(mapLibreZoom).toBeVisible({ timeout: 10_000 })
    await expect(mapLibreZoom).toHaveClass(/map-control-button/)
    await expect(mapLibreZoom).toHaveCSS('width', '46px')
    await expect(mapLibreZoom).toHaveCSS('height', '46px')
    await expect(page.locator('.maplibregl-ctrl-group')).toHaveCSS('margin', '0px')
    await mapLibreZoom.hover()
    await expect(mapLibreZoom).toHaveCSS('background-color', railHoverStyle.background)
    await expect(mapLibreZoom).toHaveCSS('border-color', railHoverStyle.border)
    await expect(mapLibreZoom).toHaveCSS('box-shadow', railHoverStyle.shadow)
    const mapLibreBox = await mapLibreZoom.boundingBox()
    expect(mapLibreBox?.x).toBeCloseTo(leafletBox?.x ?? 0, 0)
    expect(mapLibreBox?.y).toBeCloseTo(leafletBox?.y ?? 0, 0)
  })
